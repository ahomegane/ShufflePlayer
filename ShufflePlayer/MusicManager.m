...
- (id)init
{
    ...
    if (self) {
        ...
        NSMutableDictionary* tmp = [@{
                                        @"Techno" : @[
                                            @"techno,drumn,minimaltechno"
                                        ],
                                        @"House" : @[
                                            @"house,disco,techhouse"
                                        ],
                                        @"HipHop / BreakBeats" : @[
                                            @"hiphop,breakbeats,rap,triphop"
                                        ],
                                        @"Electronica / Experimental" : @[
                                        ...
                                        @"Soul / R&B" : @[
                                            @"soul,r&b,blues,nujazz"
                                        ],
                                        @"Jazz / Funk" : @[
                                            @"jazz,funk,piano,afro"
                                        ],
                                        @"Acoustic / World" : @[
         ...
         withInitFlag:(BOOL)isInit
{
        ...
        ^(NSURLResponse * response, NSData * data, NSError * error)
    {
        ...
        [self.delegate changeGenreComplete:[_tracks count]
                              withInitFlag:isInit];
    };
    NSLog(@"%@", params);
...
- (NSMutableArray*)filterTracks:(NSMutableArray*)tracks
{
    for (int i = 0; i < [tracks count]; i++) {
        ...
        if (!streamable || [format isEqualToString:@"wav"] || ![sharing isEqualToString:@"public"]) {
...
- (NSMutableArray*)randomSortTracks:(NSMutableArray*)tracks
{
    ...
    for (int i = count - 1; i > 0; i--) {
        ...
        [tracks exchangeObjectAtIndex:i
                    withObjectAtIndex:randomNum];
    }
    return tracks;
}
