...
- (void)viewDidLoad
{
    ...
    self.genreListVC.genreData = self.musicManager.genreNameList;
    ...
    // ジェンルリクエスト
    NSString* selectedGenre = [self.musicManager restoreSelectedGenreFromUserDefault];
    ...
    [self.musicManager changeGenre:selectedGenre
...
- (void)initElement
{
    ...
    UIColor* bgColorAlpha = [UIColor colorWithRed:0
                                            green:0
                                             blue:0
                                            alpha:0.7];
    ...
    [genreButton setImage:genreImage
                 forState:UIControlStateNormal];
    ...
    [alarmButton setImage:alarmImage
                 forState:UIControlStateNormal];
    ...
    trackScrollViewFrame.origin.x = (_trackScrollViewIndex - 1) * trackScrollViewFrame.size.width;
...
- (void)resetScrollView
{
    ...
    CGSize contentSize = CGSizeMake(_currentTrackScrollView.frame.size.width * _tracksCount,
...
- (void)touchGenreButton:(id)sender
{
    ...
    [self presentViewController:self.genreListVC
                       animated:YES
                     completion:nil];
}
...
- (void)touchAlarmButton:(id)sender
{
    ...
    [self presentViewController:self.alarmVC
                       animated:YES
                     completion:nil];
}
...
- (void)hideGenreView
{
    [self.genreListVC dismissViewControllerAnimated:YES
                                         completion:nil];
}
...
- (void)playAlarm
{
    [self.musicManager changeGenre:@"All"
...
- (void)hideAlarmView
{
    [self.alarmVC dismissViewControllerAnimated:YES
                                     completion:nil];
}
