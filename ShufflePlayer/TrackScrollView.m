...
- (void)initElement
{
    ...
    _waveformSequenceView.backgroundColor = [UIColor colorWithRed:0.310
                                                            green:0.310
                                                             blue:0.310
                                                            alpha:1.0];
    ...
    _titleButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light"
                                                   size:14]; //UltraLight
    ...
    [_likeButton setImage:_likeImage
                 forState:UIControlStateNormal];
...
- (void)setTrackInfo:(NSDictionary*)track
{
    ...
    [_titleButton setTitle:self.title
                  forState:UIControlStateNormal];
    ...
    _waveformSequenceView.frame = CGRectMake(rect.origin.x, rect.origin.y, 0, rect.size.height);
...
{
    CGRect rect = _waveformSequenceView.frame;
    _waveformSequenceView.frame = CGRectMake(rect.origin.x, rect.origin.y, _waveformArea.frame.size.width * (currentTime / duration),
                                             rect.size.height);
}
...
- (void)touchLikeButton:(id)sender
{
    ...
    [_accountManager sendLike:trackId method:@"post" withCompleteCallback:^(NSError * error)
    {
        if (SC_CANCELED(error)) {
            ...
            [_likeButton setImage:_likeImageOn
                         forState:UIControlStateNormal];
            button.tag = 1;
        }
    }];
